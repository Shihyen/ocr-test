...
class MongoUtil(object):
    ...
    def aggregate_pipeline(self, pipeline):
        ...
        try:
            ...
            # r = {"data": list(result)}
            # print("bsonsize:" + str(len(bson.BSON.encode(r))))
        except Exception as e:
    ...
    #                                                                         #
    ###########################################################################
    # call by Statistic, HistPrice, Tracklist
    ...
    # call by Statistic, FundDetails
    def get_fund_latest_price(self, howfund_list):
        # query mongodb
        self.setCollection("API_fundData")
        _post = self.find({"howfundId": {"$in": howfund_list},"navStatistics.RrThisYear":{"$exists": True}},
                          {"_id": 0, "howfundId": 1,
                           "navStatistics.RrThisYear": 1, "navStatistics.RrThisYear": 1})
        ...
        _df['navStatistics'] = _df['navStatistics'].fillna('')
    ...
    #Call by Statistics, HistoryPrice
    ...
    def list_fund_by_category(self, level1, level2, level3=None, level4=None):
        ...
        nav_df = nav_df.replace(['nan', '', 'None', list([])], np.nan)
    ...
    def get_fund_latest_30_price(self, howfund_id):
        ...
        df = df.fillna(method=configure.dataframe_fillna_method)
        df = df.rename({"nav": "price"}, axis="columns")
        return df
